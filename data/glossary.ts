...
export const glossary: Term[] = [
  ...
  // --------------------------------------------------------- SDV vocabulary
  {
    ...
    aliases: ['zone controller', 'zonal'],
    short: 'Grouping the wiring by where things are in the car, rather than by what they do.',
    long: 'Instead of a lighting ECU wired to every lamp in the vehicle, a zonal design puts a small controller in each corner of the car. It connects to whatever is physically near it and links back to a central computer over one high-speed cable. Wiring gets dramatically shorter and lighter; the intelligence moves to the centre.',
    analogy:
  ...
  {
    term: 'central compute',
    aliases: ['HPC', 'high performance compute', 'domain controller'],
    short: 'One powerful computer running many vehicle functions that used to have their own boxes.',
    long: 'Rather than a hundred small controllers, a few capable processors run the software for many functions at once. This is what makes a function updatable, replaceable and addable after the vehicle ships.',
    category: 'Standards',
    related: ['zonal architecture', 'hypervisor'],
  },
  {
    term: 'DDS',
    aliases: ['Data Distribution Service'],
    short: 'A publish-subscribe messaging standard used where large amounts of data must move with predictable timing.',
    long: 'Where SOME/IP suits request-response service calls, DDS suits continuous high-volume streams with quality-of-service guarantees — which is why it appears in ADAS and robotics rather than body electronics.',
    category: 'Standards',
    related: ['SOME/IP', 'service-oriented architecture'],
  },
  {
    term: 'TSN',
    aliases: ['Time-Sensitive Networking', 'Automotive Ethernet'],
    short: 'Extensions that make ordinary Ethernet deliver messages on a guaranteed schedule.',
    long: 'Standard Ethernet is fast but makes no promise about when a packet arrives. Time-Sensitive Networking adds scheduling and reservation so a control message cannot be delayed by a video stream — which is what a vehicle needs before it can replace CAN with Ethernet.',
    category: 'Standards',
    related: ['CAN', 'SOME/IP'],
  },
  {
    term: 'service-oriented architecture',
    aliases: ['SOA', 'service oriented'],
    short: 'Components offer named capabilities that others discover and call, instead of broadcasting values.',
    long: 'A seat controller offers a SeatService with a setPosition method; anything that needs it discovers the service at runtime and calls it. Adding a new consumer requires no network redesign, which is the property that lets features be added after production.',
    category: 'Standards',
    related: ['SOME/IP', 'SDV'],
  },
  {
    term: 'virtual ECU',
    aliases: ['vECU', 'SIL'],
    short: 'A software copy of a vehicle computer, running on a normal machine with no hardware involved.',
    long: 'A virtual ECU lets the real production software be tested on a laptop or a build server. It is the foundation of shift-left testing: defects that would otherwise need a physical vehicle can be found in a pipeline run.',
    analogy:
    ...
    category: 'Process',
    related: ['digital twin', 'SDV'],
  },
  {
    term: 'digital twin',
    short: 'A simulated model of a specific vehicle, kept in step with the real one.',
    long: 'A digital twin mirrors a real vehicle\'s configuration and state closely enough to reproduce its behaviour. It is used to test an update against what a customer actually has, rather than against a generic reference build.',
    category: 'Process',
    related: ['virtual ECU'],
  },
  {
    term: 'feature on demand',
    aliases: ['FoD', 'functions on demand'],
    short: 'A capability already present in the vehicle, unlocked after purchase.',
    long: 'The hardware and software shipped with the car; buying the feature issues a signed entitlement the vehicle verifies locally. Commercially attractive and reputationally sensitive — customers object to paying for hardware they already own.',
    category: 'Process',
    related: ['OTA', 'SDV'],
  },
  {
    term: 'AUTOSAR Adaptive',
    aliases: ['Adaptive Platform'],
    short: 'The version of AUTOSAR built for powerful processors and service-based communication.',
    long: 'Classic AUTOSAR targets small microcontrollers with fixed scheduling. Adaptive targets microprocessors running POSIX operating systems, with dynamic scheduling and SOME/IP services. Both exist in the same vehicle, doing different jobs.',
    category: 'Standards',
    related: ['AUTOSAR', 'SOME/IP'],
  },
  {
    term: 'shift left',
    short: 'Finding defects earlier, where they are cheaper to fix.',
    long: 'Moving testing from late stages that need physical vehicles to early stages that run in a build pipeline. In vehicle software the cost difference between the two is enormous — minutes versus a scheduled test slot.',
    category: 'Process',
    related: ['virtual ECU'],
  },
  {
    term: 'freedom from interference',
    short: 'A guarantee that one part of a system cannot disturb a more safety-critical part.',
    long: 'When a safety-rated function shares hardware with an unrated one, functional safety standards require proof that the unrated one cannot affect it — through memory, timing or shared resources. It is the requirement that hypervisor partitioning exists to satisfy.',
    category: 'Security',
    related: ['hypervisor', 'ASIL'],
  },
  {
    term: 'zero trust',
    short: 'Assuming no component is trustworthy just because it is inside the vehicle.',
    long: 'Traditionally anything on the vehicle network was trusted implicitly. Zero trust replaces that with authentication and authorisation between components, so a compromised part cannot simply command the others.',
    category: 'Security',
    related: ['SDV'],
  },
  {
    term: 'AUTOSAR Classic',
    aliases: ['Classic Platform', 'AUTOSAR CP'],
    short: 'The fixed, statically configured AUTOSAR platform for small real-time control ECUs.',
    long: 'Everything is decided at build time — tasks, memory, and which signal goes where. The result is a binary with predictable timing and no dynamic allocation, running on a microcontroller with no operating system in the desktop sense. It is what braking, steering and body ECUs are built on, and it is deliberately not updatable in the field.',
    analogy: 'A printed circuit board. Extremely reliable at exactly the job it was made for, and you cannot change what it does without making a new one.',
    category: 'Platform',
    related: ['AUTOSAR', 'AUTOSAR Adaptive', 'ECU'],
  },
  {
    term: 'POSIX',
    short: 'The standard OS interface — files, processes, threads, sockets — that Linux, QNX and Android all implement.',
    long: 'Because AUTOSAR Adaptive, QNX and Linux all present a POSIX interface, application code written against it is broadly portable between them. This is why SDV stacks can plan for the operating system to change without rewriting the applications on top.',
    category: 'Platform',
    related: ['AUTOSAR Adaptive', 'QNX'],
  },
  {
    term: 'QNX',
    aliases: ['QNX Neutrino'],
    short: 'A commercial real-time microkernel OS, widely certified for safety-critical automotive use.',
    long: 'QNX keeps drivers and services in user space rather than the kernel, so a failing driver does not take the system down. It ships with ISO 26262 certification evidence up to ASIL D, which is why it is the common choice for instrument clusters and hypervisors — the certification work is already done and paid for.',
    category: 'Platform',
    related: ['ASIL', 'hypervisor', 'cluster'],
  },
  {
    term: 'AGL',
    aliases: ['Automotive Grade Linux'],
    short: 'A Linux Foundation project producing a shared open-source Linux platform for automotive.',
    long: 'AGL provides a Yocto-based reference distribution plus common services so OEMs share a base instead of each maintaining their own. It competes with Android Automotive for the infotainment slot, trading the Play ecosystem for full control of the stack.',
    category: 'Platform',
    related: ['Yocto', 'AAOS'],
  },
  {
    term: 'Yocto',
    aliases: ['Yocto Project', 'BitBake', 'OpenEmbedded'],
    short: 'A build system that produces a custom Linux distribution from recipes.',
    long: 'Rather than shipping a distribution, Yocto builds one to your specification — you choose every package that goes in. In automotive this matters because you must account for every binary on the device for security and licence compliance, and because you are targeting a fixed board rather than a general-purpose computer.',
    category: 'Build',
    related: ['AGL'],
  },
  {
    term: 'HIL',
    aliases: ['hardware-in-the-loop', 'HIL rig'],
    short: 'A test bench where real ECU hardware runs against a simulated vehicle.',
    long: 'The ECU under test believes it is in a car: its bus inputs come from a real-time simulation of the vehicle rather than from actual sensors. HIL catches timing and electrical problems that pure software simulation cannot, but rigs are expensive and scarce, which is why teams push as much testing as possible down to SIL first.',
    category: 'Process',
    related: ['virtual ECU', 'shift left'],
  },
  {
    term: 'UNECE R155',
    aliases: ['R155', 'R156', 'CSMS'],
    short: 'UN regulations requiring a certified cybersecurity and software-update management system to sell a vehicle.',
    long: 'R155 covers cybersecurity management, R156 covers software update management. They are type-approval requirements in the EU, UK, Japan, Korea and elsewhere — without them a vehicle cannot legally be sold. They turned security and update process from good practice into a condition of doing business, which is why these topics now have budget and audits attached.',
    category: 'Security',
    related: ['OTA', 'SBOM', 'zero trust'],
  },
  {
    term: 'SBOM',
    aliases: ['software bill of materials'],
    short: 'A machine-readable list of every software component in a build, with versions.',
    long: 'When a vulnerability is announced, the first question is "are we affected?" Without an SBOM that takes weeks of archaeology across suppliers. With one it is a query. Regulation and customer contracts increasingly require them, and build systems are expected to emit one automatically.',
    category: 'Security',
    related: ['UNECE R155', 'OTA'],
  },
  {
    term: 'MISRA C',
    aliases: ['MISRA'],
    short: 'A coding-standard subset of C that bans constructs prone to undefined behaviour.',
    long: 'Safety-critical automotive C is written against MISRA and checked by a static analyser. The rules remove things like dynamic allocation after startup and implicit type conversions — not because they are always wrong, but because their failure modes are hard to argue about in a safety case.',
    category: 'Process',
    related: ['ASIL', 'AUTOSAR Classic'],
  },
  {
    term: 'edge computing',
    aliases: ['on-vehicle processing'],
    short: 'Processing data where it is produced rather than shipping it to a datacentre.',
    long: 'In a vehicle the edge is the car itself. Anything with a latency budget under a few hundred milliseconds, anything that must work without connectivity, and anything expensive to transmit — camera frames especially — is processed on the vehicle. The cloud gets summaries, not raw feeds.',
    category: 'Vehicle data',
    related: ['central compute', 'SDV'],
  },
]
