...
<Gotcha title="SecurityException fires at the call site, not at install">
Calling `CarPropertyManager` for a property your app lacks permission for
throws a real `SecurityException` right where you called
`getIntProperty`/`setIntProperty` — not at install time, and not silently.
Always verify the permission is both declared in the manifest **and**, for a
`Dangerous`-level permission, actually granted at runtime. A
`Signature|Privileged` permission like the one above is a different case
entirely: an ordinary app cannot obtain it no matter what it declares — only a
platform-signed or allowlisted privileged app can hold it.
</Gotcha>
