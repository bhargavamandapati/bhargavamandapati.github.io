...
```bash title="Reliable: full rebuild and reflash"
m -j
emulator -wipe-data -no-snapshot &
adb wait-for-device && sleep 40
```

```bash title="Faster, but not dependable — try it first while iterating, fall back above if it does not stick"
# A priv-app this deep in the platform does not reliably hot-swap this way:
# the running Car Service process is not restarted, and the install can
# fail on signature grounds.
adb install-multiple -r $ANDROID_PRODUCT_OUT/system/priv-app/CarService/*.apk
```
...
<Gotcha title="This call throws until two tutorials from now">
`setMassageIntensity` enforces `com.oem.vega.permission.CONTROL_SEAT_MASSAGE`,
and nothing in this tutorial declares that permission — it is defined in the
permissions tutorial, two steps ahead in this series. Until then, expect a
`SecurityException` from every caller, including a platform-signed one. That
is expected, not a bug in what you just built.
</Gotcha>
